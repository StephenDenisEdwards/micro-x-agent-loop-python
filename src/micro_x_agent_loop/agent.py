...
from micro_x_agent_loop.agent_listener import AgentEventListener
...
class Agent:
    ...
    def __init__(self, config: AgentConfig):
        ...
        # Compaction callback wiring is deferred until self._event_listener
        # is created (see "Pure-observability listener" below).
        ...
        # --- Pure-observability listener ---
        # Owns the metric-emission and lifecycle-event methods. Agent's
        # on_* methods that don't mutate _messages / checkpoint state
        # delegate to this listener.
        self._event_listener = AgentEventListener(
            memory=self._memory,
            obs=self._obs,
            session_accumulator=self._session_accumulator,
            metrics_enabled=self._metrics_enabled,
            turn_number_provider=lambda: self._turn_number,
            compaction_tokens_handler=(
                self._compaction_strategy.update_actual_tokens
                if isinstance(self._compaction_strategy, SummarizeCompactionStrategy)
                else None
            ),
            budget_check=self._check_budget_warning,
        )
        if self._metrics_enabled and isinstance(self._compaction_strategy, SummarizeCompactionStrategy):
            self._compaction_strategy._on_compaction_completed = (
                self._event_listener.on_compaction_completed
            )
    ...
    # -- TurnEvents protocol: metrics (delegated to AgentEventListener) --

    def on_api_call_completed(self, usage: UsageResult, call_type: str) -> None:
        self._event_listener.on_api_call_completed(usage, call_type)

    def on_api_call_failed(
        self, *, model: str, provider: str, call_type: str, error: BaseException
    ) -> None:
        self._event_listener.on_api_call_failed(
            model=model, provider=provider, call_type=call_type, error=error,
        )

    def on_turn_cap_reached(self, iterations: int) -> None:
        self._event_listener.on_turn_cap_reached(iterations)
    ...
    def on_tool_executed(
        ...
        *,
        ...
        self._event_listener.on_tool_executed(
            tool_name, result_chars, duration_ms, is_error, was_summarized=was_summarized,
        )
    ...
    def on_subagent_completed(
        ...
        *,
        ...
        self._event_listener.on_subagent_completed(
    ...
    def on_record_tool_call(
        ...
        *,
        ...
        self._event_listener.on_record_tool_call(
    ...
    def on_llm_call(
        ...
        *,
        ...
        self._event_listener.on_llm_call(
