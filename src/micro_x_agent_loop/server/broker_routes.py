"""Broker API routes — jobs, runs, HITL questions, webhook triggers."""
...
def create_broker_router(
    ...
    """Build an APIRouter with all broker endpoints.

    These routes mirror the original ``WebhookServer`` but run inside the
    unified API server rather than a standalone process.
    """
    router = APIRouter()
    ...
    async def get_question(run_id: str, question_id: str):  # type: ignore[no-untyped-def]
        ...
        if q is None or q["run_id"] != run_id:
            return Response(status_code=404, content='{"error": "Question not found"}', media_type="application/json")
        return q
    ...
    async def answer_question(run_id: str, question_id: str, request: Request):  # type: ignore[no-untyped-def]
        ...
        if not answer:
            return Response(status_code=400, content='{"error": "Answer text required"}', media_type="application/json")

        success = store.answer_question(question_id, answer=answer)
        if not success:
            return Response(
                status_code=409,
    ...
    async def list_questions(run_id: str) -> dict[str, Any]:
        """List pending questions for a run."""
        q = store.get_pending_question(run_id)
