...
class AgentCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_help_includes_new_commands(self) -> None:
        ...
        await agent._handle_local_command("/help")
        out = agent._channel.text
        self.assertIn("/session new [title]", out)
        self.assertIn("/checkpoint list [limit]", out)
        self.assertIn("/cost", out)

    async def test_unknown_local_command_message(self) -> None:
        agent = self._make_agent()
        await agent._handle_local_command("/unknown")
        self.assertIn("Unknown local command", agent._channel.text)

    async def test_session_new_and_name(self) -> None:
        agent = self._make_agent()
        await agent._handle_session_command("/session new Planning")
        await agent._handle_session_command("/session name Planning Updated")
        out = agent._channel.text
        self.assertIn("Started new session:", out)
        self.assertIn("Session named: Planning Updated", out)

    async def test_session_resume_by_name_prints_summary(self) -> None:
        agent = self._make_agent()
        await agent._handle_session_command("/session resume Session One")
        out = agent._channel.text
        self.assertIn("Resumed session Session One", out)
        self.assertIn("Session summary:", out)

    async def test_session_resume_not_found_and_ambiguous(self) -> None:
        agent = self._make_agent()
        await agent._handle_session_command("/session resume missing")
        await agent._handle_session_command("/session resume ambiguous")
        out = agent._channel.text
        self.assertIn("Session not found: missing", out)
        self.assertIn("ambiguous", out)

    async def test_session_fork_updates_active(self) -> None:
        agent = self._make_agent()
        await agent._handle_session_command("/session fork")
        out = agent._channel.text
        self.assertIn("Forked session s1 -> s-fork", out)

    async def test_checkpoint_list_and_rewind_alias(self) -> None:
        agent = self._make_agent()
        await agent._handle_checkpoint_command("/checkpoint list")
        await agent._handle_checkpoint_command("/checkpoint rewind cp1")
        out = agent._channel.text
        self.assertIn("Recent checkpoints:", out)
        self.assertIn("Rewind cp1 results:", out)

    async def test_checkpoint_usage_error_and_rewind_usage(self) -> None:
        agent = self._make_agent()
        await agent._handle_checkpoint_command("/checkpoint list x")
        await agent._handle_rewind_command("/rewind")
        out = agent._channel.text
        self.assertIn("Usage: /checkpoint list [limit]", out)
        self.assertIn("Usage: /rewind <checkpoint_id>", out)

    async def test_run_handles_max_tokens_retry_then_stop(self) -> None:
        agent = self._make_agent()
        stream_result = (
    ...
    async def test_cost_command_prints_summary(self) -> None:
        agent = self._make_agent()
        await agent._handle_cost_command("/cost")
        out = agent._channel.text
...
class ToolCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _run(self, agent: Agent, command: str) -> str:
        buf = io.StringIO()
        with redirect_stdout(buf):
            await agent._handle_local_command(command)
        return buf.getvalue()

    # -- /tool (list) --

    async def test_tool_list_groups_by_server(self) -> None:
        out = await self._run(self._make_agent(), "/tool")
    ...
    async def test_tool_list_empty(self) -> None:
        out = await self._run(self._make_agent(tools=[]), "/tool")
        self.assertIn("No tools loaded", out)

    async def test_tool_list_builtin_group(self) -> None:
        out = await self._run(self._make_agent(tools=[FakeTool(name="noop")]), "/tool")
        self.assertIn("(built-in)", out)
        self.assertIn("noop", out)

    # -- /tool <name> (details) --

    async def test_tool_details_by_full_name(self) -> None:
        out = await self._run(self._make_agent(), "/tool filesystem__bash")
        self.assertIn("Name: filesystem__bash", out)
        self.assertIn("Description: Run a shell command", out)
        self.assertIn("Mutating: True", out)

    async def test_tool_details_by_short_name(self) -> None:
        out = await self._run(self._make_agent(), "/tool read_file")
    ...
    async def test_tool_not_found(self) -> None:
        out = await self._run(self._make_agent(), "/tool nonexistent")
        self.assertIn("Tool not found: nonexistent", out)

    async def test_tool_ambiguous_short_name(self) -> None:
        out = await self._run(self._make_agent(), "/tool bash")
    ...
    async def test_tool_schema_shows_input_and_output(self) -> None:
        out = await self._run(self._make_agent(), "/tool filesystem__bash schema")
        self.assertIn("Input schema:", out)
        self.assertIn('"cmd"', out)
        self.assertIn("Output schema:", out)
        self.assertIn('"stdout"', out)

    async def test_tool_schema_no_output_schema(self) -> None:
        out = await self._run(self._make_agent(), "/tool read_file schema")
        self.assertIn("Input schema:", out)
        self.assertNotIn("Output schema:", out)

    # -- /tool <name> config --

    async def test_tool_config_shows_specific(self) -> None:
        formatting = {"filesystem__bash": {"format": "text", "field": "stdout"}}
        agent = self._make_agent(tool_formatting=formatting)
        out = await self._run(agent, "/tool filesystem__bash config")
    ...
    async def test_tool_config_shows_default(self) -> None:
        agent = self._make_agent()
        out = await self._run(agent, "/tool filesystem__bash config")
    ...
    async def test_help_includes_tool_commands(self) -> None:
        agent = self._make_agent()
        buf = io.StringIO()
        with redirect_stdout(buf):
            await agent._handle_local_command("/help")
...
class DebugCommandTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_debug_no_payloads(self) -> None:
        out = await self._run(self._make_agent(), "/debug show-api-payload")
        self.assertIn("No API payloads recorded yet", out)

    async def test_debug_show_payload(self) -> None:
        import time
        ...
        out = await self._run(agent, "/debug show-api-payload")
    ...
    async def test_debug_skips_tool_result_for_last_user_msg(self) -> None:
        import time
    ...
    async def test_debug_shows_tool_names_in_response(self) -> None:
        import time
    ...
    async def test_debug_show_payload_out_of_range(self) -> None:
        import time
        ...
        out = await self._run(agent, "/debug show-api-payload 5")
        self.assertIn("out of range", out)

    async def test_debug_usage_message(self) -> None:
        out = await self._run(self._make_agent(), "/debug")
        self.assertIn("Usage:", out)
        self.assertIn("show-api-payload", out)

    async def test_help_includes_debug_command(self) -> None:
        agent = self._make_agent()
        buf = io.StringIO()
        with redirect_stdout(buf):
            await agent._handle_local_command("/help")
