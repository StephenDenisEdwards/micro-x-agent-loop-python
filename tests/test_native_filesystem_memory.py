...
class SaveMemoryTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
    ...
    async def test_writes_and_creates_dir(self) -> None:
        r = await self.tool.execute({"file": "notes.md", "content": "hello"})
    ...
    async def test_rejects_non_md(self) -> None:
        r = await self.tool.execute({"file": "notes.txt", "content": "x"})
        self.assertTrue(r.is_error)
        self.assertIn("Only .md files are allowed", r.text)

    async def test_rejects_traversal(self) -> None:
        for bad in ("../evil.md", "sub/notes.md", "a\\b.md", "..md.md/../x.md"):
            r = await self.tool.execute({"file": bad, "content": "x"})
            self.assertTrue(r.is_error, bad)
            self.assertIn("plain filename", r.text)

    async def test_memory_md_under_cap_no_warning(self) -> None:
        r = await self.tool.execute({"file": "MEMORY.md", "content": "a\nb\nc\n"})
        self.assertFalse(r.is_error)
        self.assertEqual(r.structured["line_count"], 3)  # trailing \n not counted
        self.assertNotIn("warning", r.structured)

    async def test_memory_md_over_cap_warns(self) -> None:
    ...
    async def test_non_index_md_no_line_count(self) -> None:
        r = await self.tool.execute({"file": "topic.md", "content": "x\ny\nz"})
