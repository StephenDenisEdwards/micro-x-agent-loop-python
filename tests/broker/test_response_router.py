...
class ResponseRouterTests(unittest.IsolatedAsyncioTestCase):
    async def test_channel_none_returns_true(self) -> None:
        router = ResponseRouter({})
        store = _make_store()
        result_obj = RunResult(exit_code=0, stdout="", stderr="")
        result = await router.route("run1", "none", None, result_obj, store)
        self.assertTrue(result)
        store.mark_response_sent.assert_not_called()

    async def test_known_channel_success(self) -> None:
        ...
        sent = await router.route("run-abc123", "http", "http://target", result_obj, store)
        self.assertTrue(sent)
        store.mark_response_sent.assert_called_once_with("run-abc123")
        store.mark_response_failed.assert_not_called()

    async def test_known_channel_failure_falls_back_to_log(self) -> None:
        adapter = _make_adapter(send_result=False)
        log_adapter = _make_adapter(send_result=True)
        router = ResponseRouter({"http": adapter, "log": log_adapter})
        store = _make_store()
        result_obj = RunResult(exit_code=1, stdout="", stderr="fail")
        sent = await router.route("run-abc123", "http", None, result_obj, store)
        self.assertFalse(sent)
        store.mark_response_failed.assert_called_once()
        log_adapter.send_response.assert_called_once()

    async def test_unknown_channel_falls_back_to_log(self) -> None:
        log_adapter = _make_adapter(send_result=True)
        router = ResponseRouter({"log": log_adapter})
        store = _make_store()
        result_obj = RunResult(exit_code=0, stdout="ok", stderr="")
        sent = await router.route("run1", "unknown_channel", None, result_obj, store)
        self.assertTrue(sent)
        log_adapter.send_response.assert_called_once()

    async def test_unknown_channel_no_log_returns_false(self) -> None:
        router = ResponseRouter({})
        store = _make_store()
        result_obj = RunResult(exit_code=0, stdout="", stderr="")
        sent = await router.route("run1", "missing", None, result_obj, store)
        self.assertFalse(sent)

    async def test_adapter_exception_marks_failed(self) -> None:
        adapter = MagicMock()
        ...
        store = _make_store()
        result_obj = RunResult(exit_code=0, stdout="", stderr="")
        sent = await router.route("run1", "http", "target", result_obj, store)
        self.assertFalse(sent)
        store.mark_response_failed.assert_called_once()

    async def test_log_channel_success_no_fallback(self) -> None:
        log_adapter = _make_adapter(send_result=False)
        router = ResponseRouter({"log": log_adapter})
        store = _make_store()
        result_obj = RunResult(exit_code=0, stdout="", stderr="")
        sent = await router.route("run1", "log", None, result_obj, store)
        self.assertFalse(sent)
