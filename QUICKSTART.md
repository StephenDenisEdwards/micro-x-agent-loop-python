...
Get Micro-X Agent running in 2 minutes.
...
- [Google OAuth credentials](#gmail-setup) for Gmail and Calendar tools
- [Brave Search API key](https://brave.com/search/api/) for the `web_search` tool
- Anthropic Admin API key (`sk-ant-admin...`) for the `anthropic_usage` tool
- GitHub personal access token (`GITHUB_TOKEN`) for `github_*` tools
- Deepgram API key (`DEEPGRAM_API_KEY`) for Interview Assist transcription/STT tools
- [.NET 10 SDK](https://dotnet.microsoft.com/download) for the system-info MCP server (in the shared [mcp-servers](https://github.com/StephenDenisEdwards/mcp-servers) repo)
- [Go 1.21+](https://go.dev/dl/) and a C compiler (GCC) for the WhatsApp MCP server
- Local clone of `interview-assist-2` for Interview Assist MCP tools
...
The system-info MCP server lives in the shared [mcp-servers](https://github.com/StephenDenisEdwards/mcp-servers) repository.
...
### Interview Assist server (TypeScript)
