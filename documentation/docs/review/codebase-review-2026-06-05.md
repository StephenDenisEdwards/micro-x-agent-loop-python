...
| T2-2 | Split `commands/command_handler.py` per command group with a registry | `commands/command_handler.py` (1,062 LOC) | ✅ Done | Added `commands/command_context.py` (read-only dataclass bundling the 16 collaborators). Extracted all 14 command groups into per-command modules: `help_command.py` (94 LOC), `console_command.py` (29 LOC), `cost_command.py` (50 LOC), `replay_command.py` (32 LOC), `memory_command.py` (85 LOC), `codegen_command.py` (113 LOC), `tools_command.py` (32 LOC), `tool_command.py` (210 LOC), `debug_command.py` (96 LOC), `session_command.py` (116 LOC), `checkpoint_command.py` (60 LOC), `routing_command.py` (106 LOC), `voice_command_handler.py` (73 LOC), `compact_command.py` (27 LOC). `command_handler.py` shrunk 1,062 → 171 LOC (-84%) and is now a pure delegating facade. Backward-compat property accessors on `CommandHandler` (`_memory`, `_tool_map`, `_tool_result_formatter`) preserve the test surface. |
...
| T2-6 | Move 30+ eval/testing config files out of repo root into `configs/evals/` | `config-*.json` (36 files) | ✅ Done | Root went from 36 → 3 config files. Layout: `configs/profiles/` (23 — all `config-standard-*` plus `config-optimal-anthropic.json`), `configs/evals/` (3 — eval configs), `configs/testing/` (7 — testing configs). Entry-point configs stay at root: `config.json`, `config-base.json`, `config-baseline.json`, `config-starter.json`. Each moved file's `Base` reference rewritten from `"config-base.json"` to `"../../config-base.json"`. `config-standard-ollama.json`'s `ConfigFile` indirection updated to the new path. `config-starter.json.bak` deleted. Test references updated in `tests/test_app_config.py`, `tests/providers/test_gemma_via_gemini.py`, `tests/evals/` (4 files). Doc references updated across `documentation/`, `tools/windows-vhs/README.md`, demo `.tape` files, and `README.md`. CLAUDE.md updated to document the new layout. |
...
| T3-2 | Add `tests/integration/test_agent_loop.py` covering full multi-turn / tool / sub-agent / ask_user path with `FakeStreamProvider` | `tests/integration/` | ✅ Done | New `tests/integration/test_agent_loop.py` (6 tests) drives a real `Agent` with only the LLM provider faked — TurnEngine, pseudo-tool dispatch, AgentChannel, TurnEvents, memory facade, metrics are all real. Covers: single-turn text-only response; multi-turn tool call → tool result → final text; multiple tools in one response (results merged in original order); `ask_user` routed through the channel; iteration cap → `on_turn_cap_reached`; tool exception → `is_error: True` tool_result. |
...
| Tier 2 (architecture) | 8 | 0 | All Tier 2 items done. T2-5 partial (unused subset dropped; final flat-vs-nested decision still implicitly open). |
| Tier 3 (test quality) | 7 | 0 | All Tier 3 items done. |
| Tier 4 (hygiene) | 6 | 0 | All Tier 4 items done. |
| **Total** | **30** | **0** | All 30 review items addressed. |
