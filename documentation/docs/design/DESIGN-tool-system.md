...
| `filesystem__delete_file` | Strict | Returns `[path]` from tool input — file is snapshotted before unlink so `/rewind` can restore it |
...
|--------|-------|----------------|
| `filesystem` | bash, read_file, write_file, append_file, edit_file, delete_file, save_memory, grep, glob | `FILESYSTEM_WORKING_DIR`, `FILESYSTEM_ALLOWED_DIRS`, `USER_MEMORY_DIR` |
