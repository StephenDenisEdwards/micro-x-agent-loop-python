...
- `configs/profiles/config-standard-no-summarization.json` — recommended for general use; all cost savings except tool result summarization
...
>
> Use `configs/profiles/config-standard-no-summarization.json` for general assistant workloads. Only enable summarization for loss-tolerant tasks (e.g., log analysis where the gist suffices).
