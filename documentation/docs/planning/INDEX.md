...
| **29** | [Observability](PLAN-observability.md) | **Largely Implemented** | — | — | Production-grade observability + session step-through. **All 8 phases (0–7) Implemented 2026-06-03:** 0 unify emit path ([ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md)); 1 step-through MVP (`llm.call`/`routing.decision`/`mode.analyzed`/`session.config`, `system_prompts`, truncation cols, `code_sha`/`config_hash`); 2 `/replay`; 3 PII redaction; 4 OTel exporter (`[otel]` extra); 5 rolling-window alerting; 6 online LLM-judge eval + `/feedback` + `quality_signal`; 7 cost rollups (`cost_rollups`, `user_id`) + sampling policy. Phase 2 extended 2026-06-04 with verbatim `/replay --full` + a full-screen TUI trace browser (`Ctrl+R`/`/trace`: tree + Markdown detail). Deferred within-phase: `thinking` capture, per-table retention, broker scheduling of eval, `tool_outputs_raw` blob store. |
| **30** | [Gemma Model Support](PLAN-gemma-model-support.md) | **In Progress** | — | — | **Phase 1 (Path C, Google-hosted) Completed 2026-05-30** — new `configs/profiles/config-standard-gemma-cloud.json` profile (defaults to `gemma-3-12b-it` for free-tier safety), `tests/providers/test_gemma_via_gemini.py` offline + live-smoke tests. **Phase 2 (Path A, Ollama local) Completed 2026-05-29** — `SystemPromptCompact`/`SystemPromptExtras` config surface, `gemma_unparsed.*` metrics, `configs/profiles/config-standard-ollama-gemma3.json` profile. Live-validated end-to-end on RTX 3050 Ti / Ollama 0.24. **Headline local model swapped from `gemma3:4b` to `orieg/gemma3-tools:4b-ft`** after discovering Ollama hard-rejects `tools=` for stock gemma3. **Phase 3 (vLLM) postponed** — not viable on 4 GB Windows hardware. Phase 4 (native GemmaProvider) not triggered. |
...
| [Gemma Model Support](PLAN-gemma-model-support.md) | In Progress | Phase 1 (Path C) Completed 2026-05-30: `configs/profiles/config-standard-gemma-cloud.json` profile + offline tests; live smoke gated on GEMINI_API_KEY. Phase 2 (Path A) Completed 2026-05-29: `orieg/gemma3-tools:4b-ft` on Ollama 0.24 (stock `gemma3:4b` rejected). Phase 3 (vLLM) postponed — not viable on 4 GB Windows hardware. Phase 4 (native GemmaProvider) not triggered. |
