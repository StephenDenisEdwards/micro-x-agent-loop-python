...
| **27** | [Filesystem Navigation](PLAN-filesystem-navigation.md) | **In Progress** | — | — | Make the agent fluent at FS work à la Claude Code. **Shipped 2026-05-09:** Phases 1, 2, 2b, 3, 3b — full file-tool surface (`read_file` line-numbered + `offset`/`limit`, `write_file` / `append_file` / `edit_file` / `delete_file` all path-contained and checkpoint-tracked), opinionated descriptions, FS-navigation directive in system prompt. Closes the file-tools half of [ISSUE-005](../issues/ISSUE-005-bash-tool-bypasses-path-policy.md) §75 in full. Remaining: Phase 4 (bash containment — closes ISSUE-005 in its accident-prevention scope), Phase 5 (explore-sub-agent eval set). ~2 days remaining. |
