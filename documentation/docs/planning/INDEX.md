...
| **29** | [Observability](PLAN-observability.md) | **In Progress** | — | — | Production-grade observability + session step-through. 8 phases. **Phases 0–2 Implemented 2026-06-03:** Phase 0 (unify emit path; [ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md)), Phase 1 (step-through MVP: `llm.call`/enriched `routing.decision`/`mode.analyzed`/`session.config`, `system_prompts` table, `was_truncated`/`original_chars`, `code_sha`/`config_hash`), Phase 2 (`/replay` command + `session_replay` reconstruction). Remaining (Planned): PII redaction, OTel exporter, alerting, online eval harness, cost rollups + sampling. (Deferred within-phase: Anthropic `thinking` capture, bespoke TUI trace panel.) |
...
| [Observability](PLAN-observability.md) | In Progress | Production-grade observability + session step-through. Findings audit + 8-phase delivery. Phases 0–2 (emit-path consolidation [ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md); session step-through MVP; `/replay` command) **implemented 2026-06-03**; remaining Planned: PII redaction, OTel, alerting, online evals, cost rollups + sampling. Measured against [observability-for-ai-agents.md](../best-practice/observability-for-ai-agents.md). |
