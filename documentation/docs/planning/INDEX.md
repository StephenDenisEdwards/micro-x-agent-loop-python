...
| **25** | [Shared MCP via HTTP transport](PLAN-shared-mcp-http-transport.md) | **In Progress** | — | — | Resolves [ISSUE-006](../issues/ISSUE-006-playwright-profile-contention.md). Phase 0 (spike) and Phase 1 (`mcp_manager.py` learns SSE/HTTP transport with subprocess spawn + port polling) complete; 35/35 unit tests pass. Phases 2–4 pending. |
...
| [Shared MCP via HTTP transport](PLAN-shared-mcp-http-transport.md) | In Progress | Resolves [ISSUE-006](../issues/ISSUE-006-playwright-profile-contention.md). Phases 0 (spike) and 1 (`mcp_manager.py` HTTP/SSE transport) complete; 35/35 unit tests pass. Phases 2–4 pending. |
