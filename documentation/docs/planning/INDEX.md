...
| **29** | [Observability](PLAN-observability.md) | **In Progress** | — | — | Production-grade observability + session step-through. 8 phases. **Phases 0–3 Implemented 2026-06-03:** Phase 0 (unify emit path; [ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md)), Phase 1 (step-through MVP: `llm.call`/enriched `routing.decision`/`mode.analyzed`/`session.config`, `system_prompts` table, `was_truncated`/`original_chars`, `code_sha`/`config_hash`), Phase 2 (`/replay` + `session_replay`), Phase 3 (PII/secret redaction on observability copies; `ObservabilityRedaction` config + `MICRO_X_OBSERVABILITY_UNREDACTED`). Remaining (Planned): OTel exporter, alerting, online eval harness, cost rollups + sampling. (Deferred within-phase: `thinking` capture, TUI trace panel, per-table retention.) |
...
| [Observability](PLAN-observability.md) | In Progress | Production-grade observability + session step-through. Findings audit + 8-phase delivery. Phases 0–3 (emit-path consolidation [ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md); session step-through MVP; `/replay` command; PII/secret redaction) **implemented 2026-06-03**; remaining Planned: OTel, alerting, online evals, cost rollups + sampling. Measured against [observability-for-ai-agents.md](../best-practice/observability-for-ai-agents.md). |
