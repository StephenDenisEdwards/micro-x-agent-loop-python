...
| **27** | [Filesystem Navigation](PLAN-filesystem-navigation.md) | **In Progress** | — | — | Make the agent fluent at FS work à la Claude Code. **Shipped 2026-05-09:** Phase 1 (FS-navigation directive in system prompt + opinionated descriptions for bash/glob/grep), Phase 2 (`edit_file` tool — surgical exact-string edits, CRLF preservation, atomic write, checkpoint-tracked), Phase 3 (`read_file` upgrade — `cat -n` line numbers, `offset`/`limit`, binary refusal), Phase 3b (PathPolicy migration for all three file tools — closes the file-tools half of [ISSUE-005](../issues/ISSUE-005-bash-tool-bypasses-path-policy.md) §75). Remaining: Phase 2b (`delete_file`), Phase 4 (bash containment — closes ISSUE-005 in its accident-prevention scope), Phase 5 (explore-sub-agent eval set). ~2 days remaining. |
