...
Last updated: 2026-05-29 (Gemma Model Support: Phase 1 + Phase 2 (Path A) landed and live-validated against `orieg/gemma3-tools:4b-ft` on Ollama 0.24 — `SystemPromptCompact`/`SystemPromptExtras` config surface, `gemma_unparsed.*` metrics, pricing + context-window entries, new profile config; stock `gemma3:4b` rejected by Ollama for `tools=` so the community fine-tune is now the headline model)
...
| **29** | [Gemma Model Support](PLAN-gemma-model-support.md) | **In Progress** | — | — | **Phase 1 + Phase 2 (Path A) Completed 2026-05-29** — `SystemPromptCompact`/`SystemPromptExtras` config surface, `gemma_unparsed.fenced_json` / `bare_xml` / `hallucinated_name` metrics, pricing + `TOOL_SEARCH_CONTEXT_WINDOWS` entries, `config-standard-ollama-gemma3.json` profile. Live-validated end-to-end on RTX 3050 Ti / Ollama 0.24. **Headline model swapped from `gemma3:4b` to `orieg/gemma3-tools:4b-ft`** after discovering Ollama hard-rejects `tools=` for stock gemma3 (plan §4.5 fallback became the primary). **Phase 3 (vLLM) postponed** — not viable on 4 GB Windows hardware; revisit on dev-hardware upgrade or remote vLLM endpoint. Phase 4 (native GemmaProvider) not triggered. |
...
| [Gemma Model Support](PLAN-gemma-model-support.md) | In Progress | Phase 1 + Phase 2 (Path A) Completed 2026-05-29. Headline model: `orieg/gemma3-tools:4b-ft` (Ollama 0.24 rejects `tools=` for stock `gemma3:4b`). Shipped: `SystemPromptCompact`/`SystemPromptExtras` config surface, `gemma_unparsed.*` metrics, pricing + `TOOL_SEARCH_CONTEXT_WINDOWS` entries, `config-standard-ollama-gemma3.json` profile, 44 new unit tests. Phase 3 (vLLM) postponed — not viable on 4 GB Windows hardware. Phase 4 (native GemmaProvider) not triggered. |
