...
| **25** | [Shared MCP via HTTP transport](PLAN-shared-mcp-http-transport.md) | **In Progress** | — | — | Resolves [ISSUE-006](../issues/ISSUE-006-playwright-profile-contention.md). Phase 0 (verification spike) complete — `@playwright/mcp` confirmed to support SSE on `--port` and isolate concurrent clients by default. Phases 1–4 pending. |
...
| [Shared MCP via HTTP transport](PLAN-shared-mcp-http-transport.md) | In Progress | Resolves [ISSUE-006](../issues/ISSUE-006-playwright-profile-contention.md). Phase 0 (verification spike) complete: `@playwright/mcp` supports SSE via `--port` and isolates concurrent clients by default. Phases 1–4 pending. |
