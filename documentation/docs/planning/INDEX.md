...
| **25** | [Shared MCP via HTTP transport](PLAN-shared-mcp-http-transport.md) | **In Progress** | — | — | Resolves [ISSUE-006](../issues/ISSUE-006-playwright-profile-contention.md). Phases 0–2 complete: spike, `mcp_manager.py` SSE/HTTP transport (35/35 tests), `_runtime/src/mcp-client.ts` env-var-driven SSE switch (typecheck + 39/39 task tests). Phases 3–4 pending. |
...
| [Shared MCP via HTTP transport](PLAN-shared-mcp-http-transport.md) | In Progress | Resolves [ISSUE-006](../issues/ISSUE-006-playwright-profile-contention.md). Phases 0–2 complete: verification spike, Python `mcp_manager.py` SSE/HTTP transport, TS `mcp-client.ts` env-var-driven SSE switch. Phases 3–4 (run_task env injection, config flip) pending. |
