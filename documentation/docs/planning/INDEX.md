...
| **27** | [Filesystem Navigation](PLAN-filesystem-navigation.md) | **In Progress** | — | — | Make the agent fluent at FS work à la Claude Code. PathPolicy migration for `write_file`/`append_file`/`read_file` **shipped 2026-05-09** — closes the search-gated/file-tools-open asymmetry from [ISSUE-005](../issues/ISSUE-005-bash-tool-bypasses-path-policy.md) §75 in full for the file tools. Remaining phases: prompt/description rewrite, new `edit_file` tool, `read_file` line-numbers/offset/limit, bash containment (Phase 4 — closes ISSUE-005 in its accident-prevention scope), explore-sub-agent promotion. ~3 days remaining. |
