...
| **29** | [Observability](PLAN-observability.md) | **In Progress** | — | — | Production-grade observability + session step-through. 8 phases. **Phase 0 (unify emit path; [ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md)) + Phase 1 (session step-through MVP: `llm.call`/enriched `routing.decision`/`mode.analyzed`/`session.config` events, deduped `system_prompts` table, `was_truncated`/`original_chars`, `code_sha`/`config_hash`) — Implemented 2026-06-03.** Remaining (Planned): `/replay` command, PII redaction, OTel exporter, alerting, online eval harness, cost rollups + sampling. (Anthropic `thinking` capture deferred within Phase 1.) |
...
| [Observability](PLAN-observability.md) | In Progress | Production-grade observability + session step-through. Findings audit + 8-phase delivery. Phase 0 (emit-path consolidation, [ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md) Accepted) + Phase 1 (session step-through MVP) **implemented 2026-06-03**; remaining Planned: `/replay` command, PII redaction, OTel, alerting, online evals, cost rollups + sampling. Measured against [observability-for-ai-agents.md](../best-practice/observability-for-ai-agents.md). |
