...
**Largely Implemented** — Drafted 2026-06-02 from audit of current state. **All 8 phases (0–7) implemented 2026-06-03.** Phase 0 (emit-path consolidation, [ADR-026](../architecture/decisions/ADR-026-single-event-log-projections-not-parallel-writers.md)), 1 (step-through MVP), 2 (`/replay`), 3 (PII redaction), 4 (OTel exporter), 5 (alerting), 6 (online eval + `/feedback`), 7 (cost rollups + sampling). Deferred within their phases: Anthropic `thinking` capture (1), per-table retention (3), broker scheduling of the eval job (6), `tool_outputs_raw` blob store + sampling-as-retention (7). (Phase 2's TUI trace panel + verbatim `/replay --full` were since delivered, 2026-06-04.) See [observability-for-ai-agents.md](../best-practice/observability-for-ai-agents.md) for the framework this plan is measured against.
...
- ✅ **Verbatim mode — `/replay [session_id] --full`** (added 2026-06-04). Expands each `llm.call` with the *exact* request sent to the model: the full system prompt text (from `system_prompts`), the exact messages array, and the exact tool JSON schemas. The messages + tool schemas are captured per call into `llm_requests` (+ deduped `tool_schemas`) **only when `ObservabilityVerbatimCapture=true`** (off by default — it is heavy); the full system prompt is always available. The `llm.call` event carries a `request_id` joining to the snapshot. Secret *values* are redacted unless the run was captured under `MICRO_X_OBSERVABILITY_UNREDACTED=1` — i.e. structurally verbatim always, byte-for-byte raw on demand. This closes the Phase 1 gap that only tool *names* (not schemas) were persisted.
- ✅ **TUI trace browser** (added 2026-06-04, the previously-deferred "trace view"). A dedicated full-screen `TraceScreen` (`Ctrl+R` or `/trace [session_id]`) with a lazy **tree** (sessions → turns → events) on the left and a **Markdown detail panel** on the right that formats each event by type — prose as Markdown, verbatim messages/tool schemas as fenced JSON, metrics as key/value, including the full verbatim request when captured. Read-only; `Esc` returns to the live chat untouched. Backed by a new structured model `session_replay.build_session_model(store, session_id) -> SessionModel` (turns + typed `EventNode`s with markdown detail); the text `reconstruct_session` is unchanged and shares the stream builder.
- `thinking` blocks are absent from the render because Phase 1 deferred their capture (extended thinking not enabled).

**Acceptance (met):** `/replay <session_id>` produces a complete turn-by-turn view; `--full` additionally shows the verbatim per-call request (prompt + messages + tool schemas). An engineer debugging a regression can identify the offending decision and see exactly what was sent, without leaving the agent. Covered by `tests/test_session_replay.py`, `tests/test_verbatim_replay.py` (capture on/off, redaction of the verbatim copy, `--full` expansion + missing-capture note), and `tests/test_command_router.py::test_replay`.
